...
def train(model, train_loader, val_loader, optimizer, criterion, epochs, sechduler):
    ...
    plt.tight_layout()
    plt.savefig(f'./saved_image/loss_acc_ltc1order_{epochs}.png')
    plt.cla()
...
def eval(model, val_loader, criterion):
    ...
    with torch.no_grad():
        ...
        plt.legend()
        plt.savefig(f'./saved_image/test_ltc1order_{epochs}.png')
        plt.clf()
...
if __name__ == "__main__":
    ...
    if args.train:
        ...
        torch.save(model.state_dict(), f'./checkpoints/drone_model_ltc1order_{epochs}.pth')
        eval(model, val_loader, criterion)
    if args.test:
        model.load_state_dict(torch.load(f'./checkpoints/drone_model_ltc1order_{epochs}.pth'))
        eval(model, val_loader, criterion)